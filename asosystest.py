...
"""
    Testing `asosystest` v0.23.1.

    Ampliación de Sistemas Operativos (Curso 2023/2024)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        description=f"asosystest testing module {version}.",
        epilog='Example: %(prog)s -i example.json -t 1,3-5,7'
    )
...
class AsoSysTest:
    ...
    def __init__(self, test_d, config_d):
        ...
        # Validate test keys
        valid_test_keys = ['cmt', 'timeout', 'cmd', 'out', 'rc', 'score']
        for tk in test_d.keys():
            if tk not in valid_test_keys:
                panic("Error: Invalid test key '{}'".format(tk))

        # Initialize instance variables
        self.id = AsoSysTest.id

        self.cmd = test_d.get('cmd', '')
        if isinstance(self.cmd, list):  # List of commands
            self.cmd = ' ; '.join(self.cmd)

        self.stdout = test_d.get('out', '')
        if isinstance(self.stdout, list):  # List of lines
    ...
    def print(self, debug=False):
        ...
        if debug:
            ...
            if self.status == AsoSysStatus.SUCCESS or self.status == AsoSysStatus.FAILURE:
                expected_output_l = list(self.stdout)
                produced_output_l = list(self.res.stdout)

                # First index where expected and produced outputs differ
                if self.status == AsoSysStatus.FAILURE:
                    diff = [idx for idx, (x, y) in enumerate(zip(expected_output_l, produced_output_l)) if x != y]
                    if diff:
                        diff_idx = diff[0]
                    else:
                        diff_idx = min(len(expected_output_l), len(produced_output_l))
                    expected_output_l.insert(diff_idx, "-->")
                    produced_output_l.insert(diff_idx, "-->")
                ...
                print('{}'.format(expected_output_l, end=""))
                ...
                print('{}'.format(produced_output_l, end=""))

                if (self.returncode != self.res.returncode):
                    print("{}: {}.T{:02}: Return code mismatch [{} != {}]!\n".format(
                        os.path.basename(sys.argv[0]), AsoSysTest.desc, self.id, self.returncode, self.res.returncode), end='')
